...
const DataTableScreen = () => {
  const [tableHead, setTableHead] = useState(['Weight', 'Time']);
  ...
  const [chartData, setChartData] = useState({
    ...
    datasets: [
      {
        data: [],
        color: (opacity = 1) => `#27ae60`, // optional
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get(`${baseURL}/loadcell-data`);
      ...
      if (Array.isArray(data)) {
        ...
        const dataPoints = sortedData.map(item => item.weightScale);
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.title}>Pet Weight History</Text>
      <ScrollView
        refreshControl={
      ...
      >
        ...
        <ScrollView horizontal>
          {tableData? <View style={styles.tableContainer}>
            <Table borderStyle={{ borderWidth: 2, borderColor: '#27ae60' }}>
...
const styles = StyleSheet.create({
  ...
  title: { textAlign: 'center', fontSize: 24, marginBottom: 20, fontWeight: '700', color: '#27ae60' },
  ...
  head: { height: 40, backgroundColor: '#abebc6' },
  text: { margin: 6, color: '#000', textAlign: 'center', justifyContent: 'center', fontWeight: '400' },
  noDataText: { textAlign: 'center', fontSize: 18, color: '#27ae60', marginTop: 20 }
});
